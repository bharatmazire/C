...
DWORD MyThreadProc(LPVOID param)
{
	...
	struct BUFFER *traverse; //  for hashlist traverse
	...
	// code
	modVal = db % 3;
	printf("\n For Process : %d , Hash Value of (DataBlock) %d is : %d \n",procNum, db, modVal);

	if(hashHead[modVal] != NULL)						// for safety side
		traverse = hashHead[modVal];
	...
	if (hashHead[modVal] != NULL && traverse->iBlockNumber == db )
	{
		printf("\n\n For Process %d ,  GOT THE BUFFER \n\n",procNum);
		
		// buffer present on hash queue, but it might be getting used by another process so check its status
		if (traverse->iStatus == 1 && traverse->nextFreeList == NULL && traverse->prevFreeList == NULL)	// buffer on free list,but present in hash list
		{
			printf("\n As status says, This buffer is getting used by some another process \n");
			printf("\n So Sleep .... !!!!\n");
			_sleep(1000);
			MyThreadProc(param);
			return(0);
		...
		// if buffer on free list, remove it from free list
		else if (traverse->nextFreeList != NULL && traverse->prevFreeList != NULL)		// this check is for safety. coz as status is not 1 it will be 0. 
																						// as its 0 it will be on free list. but lets not take any risk
		{
			traverse->prevFreeList->nextFreeList = traverse->nextFreeList;
			traverse->nextFreeList->prevFreeList = traverse->prevFreeList;
			*freeHead = traverse->nextFreeList;
	...
	printf("\nFor Process %d, process is working on block ....",procNum);
	_sleep(rand() % 1000);
	printf("\nFor Process %d, putting buffer on freelist !!",procNum);
...
int main()
{
	...
	struct BUFFER **HashListHead = (struct BUFFER **)malloc(HASHVAL * sizeof(struct BUFFER *));
	...
	{
		struct BUFFER *traverse;
		traverse = HashListHead[i];
		if (HashListHead[i] != NULL)
		{
			while (traverse->nextHashQueue != NULL)
			{
				traverse = traverse->nextHashQueue;
			}
			while (traverse->prevHashQueue != NULL)
			{
				free(traverse->nextHashQueue);
				traverse = traverse->prevHashQueue;
			}
			free(traverse);
			traverse = NULL;
		}
